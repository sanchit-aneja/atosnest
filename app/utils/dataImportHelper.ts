...
class DataImportHelper {
     ...
     */
    async LoadDataFromCSVs(fileTimestamp: string, context, data: { namePrefix: string, columns: string[], tableName: string }[] = CSV_FILES): Promise<boolean> {
        this.context = context;
        //No timestamp means no use to proceed
        if (!fileTimestamp) {
            throw new CustomError("NO_TIMESTAMP", "Timestamp is missing in event grid message");
        }
        try {
            ...
            for (let file of data) {
                ...
                this.context.log(
                    "LoadDataFromCSVs :: next after" + blobName
                );
            }
        } catch (error) {
            this.context.log(
                "LoadDataFromCSVs :: Failed" + error.message
            );
            throw new CustomError("LOAD_CSV_FILES_FAILED", "Timestamp is missing in event grid message, details " + error.message);
        }
        return true;
    }
    ...
    private async cleanAllTables() {
        try {
            ...
            this.context.log(
                "cleanAllTables :: Failed" + error.message
            )
            throw new CustomError("STAGING_TABLES_CLEANUP_FAILED", "Stage Table clean failed, more details" + `${error.message}`);
        }
    }
            ...
            try {
                ...
                pool.connect(async function (err, client, done) {
                    if (err) {
                        ...
                        reject(`Error ::  Failed at pool connect in copyFromSTDIN :: ${copyCommand} - ${err}`);
                        pool.end();
                        return;
                    }
                    selfContext.log('Started..!')
                    const copyQuery = client.query(copyFrom(copyCommand));
                    const blobStream = await blobHelper.getBlobStream(blobName, selfBlobServiceClient);

                    //Check blo stream
                    if (blobStream == null) {
                        done();
                        reject(`Error ::  Failed at blobStream null in copyFromSTDIN :: ${copyCommand} - ${err}`);
                        pool.end();
                        return;
                    }
                    ...
                    blobStream.on('error', async (_err) => {
                        ...
                        reject(`Error ::  Failed at blobStream in copyFromSTDIN :: ${copyCommand} - ${_err}`);
                        pool.end();
                    });
                    copyQuery.on('error', async (_err) => {
                        done();
                        reject(`Error ::  Failed at copyQuery in copyFromSTDIN :: ${copyCommand} - ${_err}`);
                        pool.end();
                    });
                ...
                selfContext.log(err);
                reject(`Error ::  Failed at in copyFromSTDIN :: ${copyCommand} - ${err}`)
            }
        });
    }
}
