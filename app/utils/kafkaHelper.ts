...
class KafkaHelper {
    ...
    constructor(context?: Context) {
        try{
            ...
            this.log(`Failed to create insteance of KafkaHelper. Reason: ${error.message}`)
        }
    }
    ...
    async sendMessageToTopic(topic: string, payload: any) {
        try {
            ...
            this.log(`Send message to ${topic}`, `payload: ${JSON.stringify(payload)}`);
            ...
            this.log(`Message is sent successfully`);
            await producer.disconnect();
        } catch (error){
            this.log(`Failed to send message to topic: ${topic} & payload: ${JSON.stringify(payload)}. Reason: ${error.message}`);
        }
    }
}
