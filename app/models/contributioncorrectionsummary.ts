...
ContributionCorrectionSummary.addHook("beforeValidate", (contributioncorrectionsummary, _options) => {
    const schema = Joi.object({
        contributionCorrectionId: Joi.string().optional().allow(null, ""),
        correctionType: Joi.string().max(1).optional().allow(null, ""),
...
ContributionCorrectionSummary.beforeCreate(async (contributioncorrectionsummary, _options) => {
    const schema = Joi.object({
