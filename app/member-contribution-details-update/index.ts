...
const httpTrigger: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    context.log('member-contribution-details-update function processing a request.');
    ...
    if (app.isNullEmpty(payload)) {
        ...
        try {
            ...
            console.log("Failed :  member-contribution-details-update ", error?.message, error?.name, error?.moreDetails)
            const data = errorHandler.mapHandleErrorResponse(
                "",
                "",
