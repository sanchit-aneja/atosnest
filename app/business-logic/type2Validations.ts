 ...
 */
     ...
     */
         actualDataChecks: {
            ...
            "EarningsPeriodEndDate": async (row, context: Context, actualRows: ContributionHeader[]) => {
                const validationError = {
                    code: "ID12.1",
                    message: "Please check the Earnings Period End Date as it doesn't match what we're expecting for this schedule."
                }
                const filterRow = actualRows.filter((data: any) => (data.earningPeriodEndDate == row.earningPeriodEndDate))
            ...
            "PaymentSource": async (row, context: Context, actualRows: ContributionHeader[]) => {
                ...
                const filterRow = actualRows.filter((data: any) => (data.earningPeriodEndDate == row.EPED && data.paymentSourceName == row.paymentSource))
                if (filterRow.length == 0) {
            ...
            "PayPeriodFrequency": async (row, context: Context, actualRows: ContributionHeader[]) => {
                ...
                const filterRow = actualRows.filter((data: any) => (data.earningPeriodEndDate == row.EPED && data.paymentSourceName == row.paymentSource && data.paymentFrequencyDesc.toLowerCase() == row.payPeriodFrequency.toLowerCase()))
                if (filterRow.length == 0) {
            ...
            "PaymentDueDate": async (row, context: Context, actualRows: ContributionHeader[]) => {
                const validationError = {
                    code: "ID15.0",
                ...
                const filterRow = actualRows.filter((data: any) => (data.earningPeriodEndDate == row.earningPeriodEndDate &&
                    ...
                    data.paymentFrequencyDesc.toLowerCase()  == row.payPeriodFrequency.toLowerCase()  &&
            ...
            "EarningsPeriodStartDate": async (row, context: Context, actualRows: ContributionHeader[]) => {
                const validationError = {
                    code: "ID15.1",
     ...
     * @returns 
     */
     executeRulesOneByOne: async function (rules: any,   row, callback: Function, errors: Array<any>) {
        try {
