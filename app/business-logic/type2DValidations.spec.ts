...
describe("Test: Business logic common Contribution details functions", () => {
    ...
    test("Should pass all validation for EffectiveDatePartialNonPayment",async () => {
        ...
        // Act
        const output1 = await Type2DValidations.rules.EffectiveDatePartialNonPayment(input1)
        const output2 = await Type2DValidations.rules.EffectiveDatePartialNonPayment(input2)
        const output3 = await Type2DValidations.rules.EffectiveDatePartialNonPayment(input3)
        // Assert
        expect(output1).toBe(null)
    ...
    test("Should pass all validation for EffectiveDateChangeGroup",async () => {
        ...
        // Act
        const output1 = await Type2DValidations.rules.EffectiveDateChangeGroup(input1)
        const output2 = await Type2DValidations.rules.EffectiveDateChangeGroup(input2)
        const output3 = await Type2DValidations.rules.EffectiveDateChangeGroup(input3)
        // Assert
        expect(output1).toBe(null)
    ...
    test("Should throw error, when you call start", async () => {
        ...
        Type2DValidations.executeRulesOneByOne = jest.fn().mockImplementation(()=> {throw new Error("Something went wrong!");})
        //Act
    ...
    test("Should return true, when you call start with no errors of executeRulesOneByOne", async () => {
        // Arrage
        const stream = Readable.from(`H,column1\nD,column_row1\nD,column_row2\nT,2,3`);
        Type2DValidations.executeRulesOneByOne = jest.fn().mockImplementation(()=> Promise.resolve([]))
        //Act
    ...
    test("Should return one error, when executeRulesOneByOne called", async ()=>{
        ...
        for(const key in Type2DValidations.rules){
            ...
            if(key == 'PensEarnings'){
                Type2DValidations.rules[key] =  jest.fn().mockImplementation(()=> Promise.resolve({
                    code: "ID25.0",
                    message: "Please remove invalid characters. The format must be numerical."
                }))
            } else{
                Type2DValidations.rules[key] =  jest.fn().mockImplementation(()=> Promise.resolve(null))
            }
        }
        ...
        // Act
        const errors = await Type2DValidations.executeRulesOneByOne(customRow, context, []);
